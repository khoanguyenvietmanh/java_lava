...
public class APathFinding {
    ...
    public void Trace_Path() //print path when reach to destination
    {
        ...
        for(int i = Path.size()-1;i >= 0 ;i--)
        {
            if(i != 0)
            {
                System.out.printf("(%d %d)--->",Path.get(i).getX()+1,Path.get(i).getY()+1);
            }
            else
            {
                System.out.printf("(%d %d)",Path.get(i).getX()+1,Path.get(i).getY()+1);
    ...
    public void find_Path()
    {
        ...
        boolean found_dest = false; // check exist path
        ...
        if(!found_dest)
        {
            System.out.println("Cannot find path to the destination!!!");
            return;
        }
    }
}
